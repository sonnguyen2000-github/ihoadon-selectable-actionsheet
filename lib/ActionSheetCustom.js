...
class ActionSheet extends React.Component {
  ...
  componentDidUpdate() {
    if (typeof this.props.onSelectedChange == "function" && this.state.visible)
      this.props.onSelectedChange(this.state.selected);
  }

  show = () => {
    this.setState({ visible: true, selected: this.props.selected ?? [] }, () => {
      setTimeout(() => {
        this._showSheet();
      });
    });
  ...
  _createButton(title, index, isCancelButton = false) {
    ...
    return (
      ...
      >
        ...
        >
          <Image source={selectIcon} style={{}} resizeMode="center" />
        </View>
      </TouchableOpacity>
    );
  }
